function [M,T,Q]=generate_tri_mesh_normal(left,right,bottom,top,h_f,degree_P)
% Yu Wei 2017-05
...
%For the nth rec, store the information of its two triangular elements whose element indices are 2n-1 and 2n.
   for n=1:N1*N2
      ...
      T(1,2*n-1)=Q(3*column-2,3*row-2);
      T(2,2*n-1)=Q(3*column+1,3*row-2); 
      T(3,2*n-1)=Q(3*column+1,3*row+1);
      T(4,2*n-1)=Q(3*column-1,3*row-2);
      T(5,2*n-1)=Q(3*column,3*row-2);
      T(6,2*n-1)=Q(3*column+1,3*row-1);
      T(7,2*n-1)=Q(3*column+1,3*row);
      T(8,2*n-1)=Q(3*column,3*row);
      T(9,2*n-1)=Q(3*column-1,3*row-1);
      T(10,2*n-1)=Q(3*column,3*row-1);



      T(1,2*n)=Q(3*column-2,3*row-2);
      T(2,2*n)=Q(3*column+1,3*row+1); 
      T(3,2*n)=Q(3*column-2,3*row+1);
      T(4,2*n)=Q(3*column-1,3*row-1);
      T(5,2*n)=Q(3*column,3*row);
      T(6,2*n)=Q(3*column,3*row+1);
      T(7,2*n)=Q(3*column-1,3*row+1);
      T(8,2*n)=Q(3*column-2,3*row);
      T(9,2*n)=Q(3*column-2,3*row-1);
      T(10,2*n)=Q(3*column-1,3*row);
