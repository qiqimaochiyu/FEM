function [boundary_nodes,boundary_edges]=generate_boundary_nodes_edges_mesh3(N1_basis,N2_basis,N1_partition,N2_partition)
...
for k=1:N1_partition
    boundary_edges(2,k)=(k-1)*4*N2_partition+1;
...
for k=N1_partition+1:N1_partition+N2_partition
    boundary_edges(2,k)=(N1_partition-1)*4*N2_partition+4*(k-N1_partition);
...
for k=N1_partition+N2_partition+1:2*N1_partition+N2_partition
    boundary_edges(2,k)=(2*N1_partition+N2_partition+1-k)*4*N2_partition-1;
...
for k=2*N1_partition+N2_partition+1:nbe
    boundary_edges(2,k)=4*(2*N1_partition+2*N2_partition+1-k)-2;
