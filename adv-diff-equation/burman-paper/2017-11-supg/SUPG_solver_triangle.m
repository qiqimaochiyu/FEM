function r=SUPG_solver_triangle(left,right,bottom,top,h_1,e,a_1,a_2)
%Yu wei 2017-0608
global mesh_type
data=example1;
...
elseif mesh_type==1 
    ...
    h=sqrt(1/2)*h_1(1)/cos(10*pi/180);
    c=h*(coth(h/(2*e))-2*e/h)/2;
elseif mesh_type==2
...
A9=A6';
B1=assemble_matrix_SUPG(e,h_1,data.a,M,T,T,T,3,3,number_of_elements,matrix_size,G_coe_ref_tri,G_point_ref_tri,1,0,1,0);
B2=assemble_matrix_SUPG(e,h_1,data.a,M,T,T,T,3,3,number_of_elements,matrix_size,G_coe_ref_tri,G_point_ref_tri,0,1,0,1);
B6=assemble_matrix_SUPG(e,h_1,data.a,M,T,T,T,3,3,number_of_elements,matrix_size,G_coe_ref_tri,G_point_ref_tri,1,0,0,1);
B9=B6';
...
s2=assemble_vector_SUPG(e,h_1,data.f,M,T,T,3,number_of_elements,vector_size,G_coe_ref_tri,G_point_ref_tri,1,0);
s3=assemble_vector_SUPG(e,h_1,data.f,M,T,T,3,number_of_elements,vector_size,G_coe_ref_tri,G_point_ref_tri,0,1);

if mesh_type==0 || mesh_type==1
    A=(c*a_1^2+e)*A1+(c*a_2^2+e)*A2+c*a_1*a_2*(A6+A9)+a_1*A4+a_2*A7;
    k=k1+c*a_1*k2+c*a_2*k3;
elseif mesh_type==2 || mesh_type==3
